...
<div class="">
	<input
		...
		class="hoverClick {FilterValue?'bg-green-600':'bg-gray-500'} h-full border-4 border-gray-800 p-2"
		on:click={(e) => {
			FilterStoreCommands.updateURL((old) => {
				//oiu.set(	old,filterName,filterTextValue)
				//let a = oiu.create();

				oiu.set(old, FilterData.name, !FilterValue);
				return old;
			});
		}}
	/>
